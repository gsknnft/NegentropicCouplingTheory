...
import { ResolutionEvent } from "../packages/coherence/coherence";
...
export class ResonanceDetector {
  ...
  detectResonance(
    ...
    if (this.isResolved(current_M, current_V)) {
      ...
      if (resonance > this.resonance_thresh && Math.abs(residual) < 0.05) {
        // Low residual threshold
    ...
    if (this.history.length >= this.max_no_event && this.events.length === 0) {
      console.log(
        "Falsification: No resolution events in window; discard candidate.",
      );
  ...
  private stdDev(arr: number[]): number {
    ...
    return Math.sqrt(
      arr.map((x) => (x - m) ** 2).reduce((a, b) => a + b, 0) / arr.length,
    );
  }
